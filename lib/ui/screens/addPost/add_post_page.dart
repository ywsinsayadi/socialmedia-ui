    ...
    if (result.isAuth) {
      ...
      if (albums.isNotEmpty) {
        List<AssetEntity> photos =
            await albums[0].getAssetListPaged(page: 1, size: 2);
    ...
    return BlocListener<PostBloc, PostState>(
      listener: (context, state) {
        if (state is LoadingPost) {
          modalLoading(context, '...ایجاد پست');
                            ...
                            Container(
                              ...
                              child: TextFormField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintText: 'یک نظر اضافه کنید',
                                  hintStyle: GoogleFonts.roboto(fontSize: 18),
                                ),
                                validator: RequiredValidator(
                                    errorText: 'فیلد الزامی است'),
                              ),
                            ),
                          ],
                        ),
                    ...
                    itemBuilder: (context, i) {
                      return InkWell(
                        onTap: () async {
                          ...
                          postBloc.add(
                            OnSelectedImageEvent(fileImage),
                          );
                        },
                        child: FutureBuilder(
                          ...
                          builder: (_, state) {
                            if (state.privacyPost == 1) {
                              return const TextCustom(
                                  text: 'همه می توانند نظر بدهند',
                                  fontSize: 16);
                            }
                            if (state.privacyPost == 2) {
                              return const TextCustom(
                                  text: 'فقط دنبال کنندگان', fontSize: 16);
                            }
                            if (state.privacyPost == 3) {
                              return const TextCustom(
                                  text: 'هیچکس', fontSize: 16);
                            }
                            return const SizedBox();
                          },
                        ),
                        ...
                        onPressed: () async {
                          AppPermission()
                              .permissionAccessGalleryMultiplesImagesNewPost(
                                  await Permission.storage.request(), context);
                        },
            ...
            onPressed: () {
              if (_keyForm.currentState!.validate()) {
                if (state.imageFileSelected != null) {
                  ...
                  modalWarning(context, 'هیچ تصویر انتخابی وجود ندارد!');
                }
              }
            },
